...
After generating frames, use them in your code (snippet):
...
proc StartExplosion(x: word, y: word) {
    ...
    // Set up first frame
    asm {
        pea expl_frame000                // arg2: sprite data pointer (pushed first)
        move.l #0,-(sp)                  // arg1: sprite slot index (pushed last)
        jsr CreateSprite
        addq.l #8,sp
        ...
        ext.l d0
        move.l d0,-(sp)                  // arg2: x
        move.l #0,-(sp)                  // arg1: sprite slot index
...
proc UpdateExplosion() {
    ...
    asm {
        ...
        move.l a0,-(sp)                  // arg2: sprite data pointer
        move.l #0,-(sp)                  // arg1: sprite slot index
...
3. **Test individual frames** - Use `sprite_importer.py` or `bob_impoerrter.py` to test single frames first
